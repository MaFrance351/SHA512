...
string SHA512(char * filename)
{
    ...
    ifstream NewFile(filename,ios_base::in);
    if(!NewFile) return "0";
    NewFile.seekg(0,ios_base::end);
    unsigned long fileSize = NewFile.tellg();
    NewFile.seekg(0,ios_base::beg);

    int tobeadded;

    // Find modded string length
    int modded = (fileSize * 8) % 1024;
    ...
    // of string length
    string lengthbits = std::bitset<128>(fileSize * 8).to_string();
    ...
    // size 1024 each
    int blocksnumber = (s1024.length() + (fileSize * 8)) / 1024;
    ...
    for (int letsgo = 0; letsgo < blocksnumber; ++letsgo) {
        ...
        string current = "";
        char temp = 0;
        if(fileSize - NewFile.tellg() < 128) {
            int newSize = fileSize - NewFile.tellg();
            for (int i = 0; i < newSize; ++i) {
                NewFile.read(&temp,1);
                // Add the character to stream
                fixedstream << bitset<8>(temp);
            }
        current = fixedstream.str();
        current += s1024;
        }
        else {
        for (int i = 0; i < 128; ++i) {
...
int main()
{
    cout << SHA512("test.txt");  //the checksum for the file provided with this program is 89c2490fc0d524dabb28bd223aaede2eac2773bf7a123ba78b2e414ca27569d24b1f47d18d5aa799c9afc681c89c2503a77e7f1cf4e4519ca3483ceef64934ce
    return 0;
}
